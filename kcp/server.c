...
int udpOutput(const char* buf, int len, ikcpcb* kcp, void* user)
{
	...
	retval = sendto(ku->sockfd, buf, len, MSG_DONTWAIT, (struct sockaddr*)&ku->cli_addr, ku->cli_addr_len);
	if(retval < 0)
	{
		perror("sendto");
		return -1;
...
int main()
{
	...
	ku.srv_addr_len = sizeof(srv_addr);

	// bind
	retval = bind(sockfd, (struct sockaddr*)&srv_addr, srv_addr_len);
	if(retval < 0)
	{
		perror("bind");
		return -1;
	...
	while (1)
	{
		...
		// recv
		len = recvfrom(sockfd, buf, BUFSIZ, MSG_DONTWAIT, (struct sockaddr*)&cli_addr, &cli_addr_len);
		if(len < 0)
		{
			continue;
		}
		ku.cli_addr = cli_addr;
		ku.cli_addr_len = cli_addr_len;
		printf("len: %d\n", len);
